...
module.exports = {
  config: {
    ...
    version: "6.0 Eren-AI FIXED",
    cooldown: 5,
    role: 1,
    description: "Premium Event Manager - Eren-AI",
    category: "system",
    usePrefix: true
  },
      ...
      if (event.reply_to_message && event.reply_to_message.document) {
        ...
        try {
          ...
          if (result.success) {
            ...
            return message.reply(makeBox('✅ INSTALLED', ['File Name: ' + fileName, 'Installed ✅ - Eren-AI']));
          }
        } catch (e) { return message.reply(makeBox('❌ FAILED', ['Failed: ' + e.message])); }
      }

      if (args.length === 0) {
        return message.reply(makeBox('⚙️ EVENT MANAGER - Eren-AI', ['install <name.js>','load <name>','unload <name>','loadall','del <name>','list','','Reply to list:','get 1 2 3','del 1 2 3']));
      }

      const action = args[0].toLowerCase();

      if (action === 'install' || action === 'add') {
        if (args.length < 2) return message.reply(makeBox('❌ USAGE', 'Usage: event install <file.js> <code|url> - Eren-AI'));
        const fileName = args[1];
        if (!fileName.endsWith('.js')) return message.reply(makeBox('❌ ERROR', 'Filename must end with.js - Eren-AI'));
        ...
        if (!code) return message.reply(makeBox('❌ ERROR', 'Provide code/URL - Eren-AI'));
        const filePath = path.join(eventsPath, fileName);
        if (fs.existsSync(filePath)) {
          const buttons = [[{ text: '✅ REPLACE', callback_data: 'evt_replace_' + fileName }, { text: '📝 RENAME', callback_data: 'evt_rename_' + fileName }],[{ text: '❌ CANCEL', callback_data: 'evt_cancel' }]];
          global.badol.onCallback.set('evt_install_' + fileName, { fileName: fileName, code: code, userId: event.from.id });
          return await api.sendMessage(event.chat.id, makeBox('⚠️ FILE EXISTS - Eren-AI', [fileName]), { reply_markup: { inline_keyboard: buttons } });
        }
        const result = global.installEventFile(fileName, code);
        if (result.success) {
          await global.reloadEvent(fileName.replace('.js', ''));
          return message.reply(makeBox('✅ INSTALLED - Eren-AI', [fileName]));
        } else return message.reply(makeBox('❌ FAILED', result.message));
      }

      if (action === 'load') {
        if(!args[1]) return message.reply(makeBox('❌ ERROR','Provide filename! - Eren-AI'));
        const r = await global.reloadEvent(args[1].replace('.js',''));
        return message.reply(r.success? makeBox('✅ LOADED - Eren-AI', [args[1]]) : makeBox('❌ FAILED', r.message));
      }
      if (action === 'unload') {
        if(!args[1]) return message.reply(makeBox('❌ ERROR','Provide filename! - Eren-AI'));
        const r = global.unloadEvent(args[1].replace('.js',''));
        return message.reply(r.success? makeBox('✅ UNLOADED - Eren-AI', [args[1]]) : makeBox('❌ FAILED', r.message));
      }
      if (action === 'loadall') {
        const files = fs.readdirSync(eventsPath).filter(f=>f.endsWith('.js'));
        let s=0; for(const f of files){ try{ await global.reloadEvent(f.replace('.js','')); s++; }catch{}}
        return message.reply(makeBox('🔄 LOADALL - Eren-AI', ['Total: ' + files.length, 'Reloaded: ' + s + ' ✅']));
      }
      if (action === 'delete' || action === 'del') {
        if(!args[1]) return message.reply(makeBox('❌ ERROR','Provide filename! - Eren-AI'));
        const r = global.deleteEventFile(args[1]);
        return message.reply(r.success? makeBox('🗑️ DELETED - Eren-AI', [args[1]]) : makeBox('❌ FAILED', r.message));
      }

      if (action === 'list') {
        const files = fs.readdirSync(eventsPath).filter(f=>f.endsWith('.js')).sort();
        const listText = files.map((n,i) => `${i+1}. ${n}`).join('\n');
        const sent = await api.sendMessage(event.chat.id, makeBox('📜 EVENT LIST ['+files.length+'] - Eren-AI', ['Total: ' + files.length,'','Reply with:','get 1 2 3','del 1 2 3','', listText]), { reply_to_message_id: event.message_id });
        global.badol.onReply.set(sent.message_id, { commandName: "event", action: "list_handle", files: files, author: event.from.id });
        return;
      }

    } catch (error) {
      return message.reply(makeBox('❌ ERROR', error.message + ' - Eren-AI'));
    }
  },
  ...
  onCallback: async function (data) {
    try {
      ...
      if (callbackData === 'evt_cancel') {
        try{ await ctx.answerCbQuery('❌ Cancelled - Eren-AI'); }catch{}
        try{ await ctx.editMessageText(makeBox('❌ CANCELLED', 'Cancelled - Eren-AI')); }catch{ try{ await api.deleteMessage(event.message.chat.id, event.message.message_id); }catch{}}
        return;
      }
      if (callbackData.startsWith('evt_replace_')) {
        ...
        if (!storedData || storedData.userId!== userId) { try{ await ctx.answerCbQuery('❌ Expired - Eren-AI'); }catch{} return; }
        try{ await ctx.answerCbQuery('✅ Replacing... - Eren-AI'); }catch{}
        const result = global.installEventFile(fileName, storedData.code);
        if (result.success) {
          try{ await global.reloadEvent(fileName.replace('.js', '')); await ctx.editMessageText(makeBox('✅ REPLACED - Eren-AI', [fileName])); }catch(e){ await ctx.editMessageText(makeBox('⚠️ WARNING - Eren-AI', ['Replaced but load failed: ' + e.message])); }
        } else {
          await ctx.editMessageText(makeBox('❌ FAILED - Eren-AI', result.message));
        }
      ...
      if (callbackData.startsWith('evt_rename_')) {
        ...
        const storedData = global.badol.onCallback.get('evt_install_' + fileName);
        if (!storedData || storedData.userId!== userId) { try{ await ctx.answerCbQuery('❌ Expired - Eren-AI'); }catch{} return; }
        try{ await ctx.answerCbQuery('📝 Rename... - Eren-AI'); }catch{}
        try{ await ctx.editMessageText(makeBox('📝 RENAME - Eren-AI', ['Reply with new filename','Must end with.js'])); }catch{}
      ...
      try{ await data.ctx.answerCbQuery('❌ Error: ' + error.message + ' - Eren-AI'); }catch{}
    }
  },
  ...
  onReply: async function (data) {
    try {
      ...
      if (replyData.action === 'list_handle') {
        ...
        if (selected.length === 0) return message.reply(makeBox('❌ ERROR', 'Invalid numbers! - Eren-AI'));
        ...
        if (cmdType === 'get') {
          for (const fileName of selected) {
            const filePath = path.join(eventsPath, fileName);
            try {
              await api.sendDocument(event.chat.id, { source: fs.readFileSync(filePath), filename: fileName }, { caption: `📄 ${fileName} - Eren-AI`, reply_to_message_id: event.message_id });
            } catch (e) {
        ...
        if (cmdType === 'del' || cmdType === 'delete') {
          ...
          return message.reply(makeBox('🗑️ DELETE RESULT - Eren-AI', lines));
        }
      ...
      if (replyData.action === 'rename_install') {
        ...
        if (!newFileName.endsWith('.js')) return message.reply(makeBox('❌ ERROR', 'Must end with.js - Eren-AI'));
        const eventsPath = path.join(__dirname, '..', 'events');
        const filePath = path.join(eventsPath, newFileName);
        if (fs.existsSync(filePath)) return message.reply(makeBox('❌ ERROR', [newFileName + ' Already exists! - Eren-AI']));
        ...
        if (result.success) {
          try{ await global.reloadEvent(newFileName.replace('.js', '')); await message.reply(makeBox('✅ INSTALLED - Eren-AI', [newFileName])); }
          catch(e){ await message.reply(makeBox('⚠️ WARNING - Eren-AI', e.message)); }
        } else await message.reply(makeBox('❌ FAILED - Eren-AI', result.message));
        global.badol.onReply.delete(event.reply_to_message.message_id);
      }
    } catch (error) { return data.message.reply(makeBox('❌ ERROR', error.message + ' - Eren-AI')); }
  }
};