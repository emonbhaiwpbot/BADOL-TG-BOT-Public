...
const ADMINONLY_FILE = path.join(ROOT_DIR, "data/adminonly.json");
...
module.exports = {
  config: {
    ...
    version: "6.4.3 Eren-AI FIXED",
    description: "Global AntiLink 5s + SpamMute Fixed + Channel Whitelist - Eren-AI",
    category: "admin",
    usePrefix: true,
    role: 2,
    cooldown: 2
  ...
  onCallback: async function ({ event, api, ctx }) {
    ...
    const isAdmin = global.config.ownerInfo?.botAdmins?.includes(String(userId)) || String(userId) === "6954597258" || global.config.ownerInfo?.mainOwner?.some(o=>String(o.id)===String(userId));
    if (!isAdmin) {
      try { await ctx.answerCbQuery("❌ Only Bot Admin! - Eren-AI", { show_alert: true }); } catch {}
      return;
    }
    ...
    if (data === "setting_role_menu" || data.startsWith("setting_role_page_")) {
      ...
      const botName = safeName(global.config.botInfo?.name || "Eren-AI", 20);
    ...
    if (data.startsWith("setting_edit_")) {
      ...
      try {
        await ctx.editMessageText(`🛠️ EDIT: /${cmdName} - Eren-AI`, {
    ...
    if (data.startsWith("setting_save_")) {
      ...
      try { await ctx.editMessageText(`✅ /${cmdName} → Role ${newRole} - Eren-AI`, { reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "setting_role_menu" }]] } }); } catch {}
      return;
    }
    ...
    if (data === "setting_system_menu") {
      try {
        ...
        await ctx.editMessageText(`📊 SYSTEM - Eren-AI\nCmds: ${cmdCount} | Users: ${totalUsers} | Groups: ${totalThreads}\n\n╰─❖─〔 𝐄𝐫𝐞𝐧-𝐀𝐈 〕─❖─╯`, { reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "setting_main" }]] } });
      } catch {}
      return;
    }

    if (data === "setting_restart") { try { await ctx.editMessageText(`🔄 Restarting Eren-AI...`); } catch {} setTimeout(()=>process.exit(2), 1200); return; }
    if (data === "setting_clear_cache") { global.badol.threadAdmins.clear(); global.badol.cooldowns.clear(); global.spamMuteMap.clear(); try { await ctx.answerCbQuery("✅ Cleared! - Eren-AI"); } catch {} return sendMainPanel(null, null, ctx, "✅ Cache Cleared! - Eren-AI\n\n"); }
    if (data === "setting_fixfiles") {
      const dataDir = path.join(ROOT_DIR, 'data');
      if (!fs.existsSync(dataDir)) fs.mkdirSync(dataDir, { recursive: true });
      const filesToFix = [{ path: path.join(dataDir, 'approvedGroups.json'), content: '[]' }, { path: path.join(dataDir, 'prefixmode.json'), content: JSON.stringify({ enabled: global.config.settings?.prefixModeEnabled || false }, null, 2) }];
      let fixed = []; filesToFix.forEach(f => { if (!fs.existsSync(f.path)) { fs.writeFileSync(f.path, f.content); fixed.push(path.basename(f.path)); } });
      try { await ctx.answerCbQuery(fixed.length? `Fixed: ${fixed.join(", ")} - Eren-AI` : "All Files OK! - Eren-AI"); } catch {}
      return;
    }
  },
  ...
  onChat: async function({ api, msg, chatId }) {
    ...
    const antilinkON = settings.antilinkGlobalEnabled === true;
    const spamMuteON = settings.spamMuteGlobalEnabled === true;

    if (!antilinkON &&!spamMuteON) return;

    const isBotAdmin = global.config.ownerInfo?.botAdmins?.includes(String(fromId)) || String(fromId) === "6954597258" || global.config.ownerInfo?.mainOwner?.some(o=>String(o.id)===String(fromId));
        ...
        if (data.count > 5) {
          ...
          try {
            ...
            if (e.message.includes("not enough rights") || e.message.toLowerCase().includes("admin")) {
              await api.sendMessage(chatId, `❌ Mute Fail: Bot কে Admin বানাও! Ban Permission দাও! - Eren-AI`).catch(()=>{});
            }
          }
        }
...
async function sendMainPanel(api, chatId, ctx, extra="") {
  const botName = safeName(global.config.botInfo?.name || "Eren-AI", 20);
  const text = `${extra||""}╭─❖─〔 ${botName} 〕─❖─╮\n│ ⚙️ CONTROL PANEL V6.4.3 - Eren-AI\n╰─❖─〔 𝐄𝐫𝐞𝐧-𝐀𝐈 〕─❖─╯`;
...
async function sendGeneralMenu(ctx) {
  const s = getSettings();
  try {
    await ctx.editMessageText(`⚙️ GENERAL SETTINGS - Eren-AI`, {
...
async function sendSecurityMenu(ctx) {
  const s = getSettings();
  try {
    await ctx.editMessageText(`🔐 SECURITY & PROTECTION (GLOBAL) - Eren-AI\n📌 ON = সব গ্রুপে কাজ করবে\n📌 OFF = সব গ্রুপে বন্ধ\n🟢 = চালু | 🔴 = বন্ধ`, {
      reply_markup: {
        inline_keyboard: [
...
async function sendMessageMenu(ctx) {
  const s = getSettings();
  try {
    await ctx.editMessageText(`💬 MESSAGE & EVENTS - Eren-AI`, {
...
function toggleSetting(key) {
  ...
  const saveConfig = () => {
    try {
      let configData = {};
      if (fs.existsSync(CONFIG_FILE)) configData = JSON.parse(fs.readFileSync(CONFIG_FILE, 'utf8'));
      else configData = global.config;
  ...
  const map = {
    ...
    adminOnly: () => { const nv=!cur.adminOnly; s.adminOnlyMode=nv; s.onlyAdmin=nv; try { fs.mkdirSync(path.dirname(ADMINONLY_FILE), { recursive: true }); fs.writeFileSync(ADMINONLY_FILE, JSON.stringify({ enabled: nv, time: Date.now() }, null, 2), 'utf8'); } catch {} },
  ...
  if (map[key]) map[key]();
  try { if (key === "prefixMode") { const pmPath = path.join(ROOT_DIR, "data/prefixmode.json"); fs.mkdirSync(path.dirname(pmPath), { recursive: true }); fs.writeFileSync(pmPath, JSON.stringify({ enabled: s.prefixModeEnabled }, null, 2)); } } catch {}
  saveConfig();
}