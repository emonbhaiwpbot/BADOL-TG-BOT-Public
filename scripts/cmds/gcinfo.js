...
module.exports = {
  ...
  BADOL: async function ({ event, api, args, message, chatId, userId, ctx }) {
    ...
    try {
      // ✅ FIXED - indicator এর বদলে Telegram typing
  ...
  onCallback: async function ({ event, api, message, ctx }) {
    ...
    if (!m) return await ctx.answerCbQuery('❌ Invalid').catch(()=>{});
    const chatId = m[1];
    ...
    if (data.startsWith('show_admins_')) {
      try {
        ...
        await edit(t, kb);
        await ctx.answerCbQuery('✅ Admins loaded!');
      } catch (e) {
