...
module.exports = {
  config: {
    ...
    version: "8.0 LIST REPLY - Eren-AI",
    cooldown: 3,
    role: 1,
    description: "Premium Command Manager - Eren-AI",
    category: "system",
    usePrefix: true
  },
  ...
  BADOL: async function ({ event, api, args, message, ctx }) {
    try {
        ...
        try {
          ...
          if (result.success) {
            try{
              ...
              try { fs.unlinkSync(path.join(__dirname, fileName)); } catch {}
      ...
      if (action === 'loadall') {
        ...
        // data path আগের মতোই - Eren.js হলেও data ফোল্ডার same
        try {
          const savePath = path.join(__dirname, "..", "data", "prefixmode.json");
          if (fs.existsSync(savePath)) {
            const saved = JSON.parse(fs.readFileSync(savePath, "utf8"));
            if (saved.mode === true || saved.enabled === true) {
      ...
      if (action === 'load') {
        try{
          if(!args[1]) return message.reply(makeBox('❌ ERROR', 'Provide filename!'));
      ...
      if (action === 'unload') {
        if(!args[1]) return message.reply(makeBox('❌ ERROR', 'Provide filename!'));
        const r = global.unloadCommand(args[1].replace('.js',''));
        return message.reply(r.success? makeBox('✅ UNLOADED', [args[1]]) : makeBox('❌ FAILED', r.message));
      }
      if (action === 'delete' || action === 'del') {
        if(!args[1]) return message.reply(makeBox('❌ ERROR', 'Provide filename!'));
        const r = global.deleteCommandFile(args[1]);
    ...
    try {
      ...
      if (callbackData.startsWith('cmd_replace_')) {
        ...
        if (result.success) {
          ...
          catch (e) { await ctx.editMessageText(makeBox('❌ LOAD FAILED', [e.message])); try { fs.unlinkSync(path.join(__dirname, fileName)); } catch {} }
        }
  ...
  onReply: async function (data) {
    try {
      const event = data.event; const message = data.message; const api = data.api || event.api || data.ctx?.telegram;
      ...
      if (replyInfo.action === 'rename_install') {
        if (replyInfo.userId && replyInfo.userId!== event.from.id) return;
        ...
        if (result.success) {
          ...
          catch (e) { await message.reply(makeBox('❌ LOAD FAILED', [e.message])); try { fs.unlinkSync(path.join(__dirname, newFileName)); } catch {} }
        }
