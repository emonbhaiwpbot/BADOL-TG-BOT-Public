...
async function getEffectivePrefix(threadId){
  let configPrefix = global.config?.botInfo?.prefix || global.config?.prefix || "/";
  try {
...
module.exports = {
  config: {
    ...
    version: "14.0 Eren-AI FIXED",
    description: "Effective Prefix - Custom or Config - Eren-AI",
    category: "islamic",
    usePrefix: false,
    cooldown: 2,
    role: 0,
  ...
  BADOL: async function ({ event, api, message, chatId }) {
    ...
    const txt = `╭─❖─〔 𝐄𝐫𝐞𝐧-𝐀𝐈 〕─❖─╮\n│ 🕌 𝗔𝗟𝗟𝗔𝗛𝗨 𝗔𝗞𝗕𝗔𝗥 - Eren-AI\n├────────────────\n│\n│ ❝ ${randomCaption} ❞\n│\n├────────────────\n│ 👤 ${toBoldItalic("User")}: ${name}\n│ 🤖 ${toBoldItalic("Bot")}: ${botName}\n│ 📌 ${toBoldItalic("Prefix")}: ${effectivePrefix}\n╰─❖─〔 𝐄𝐫𝐞𝐧-𝐀𝐈 〕─❖─╯\n\n🕋 La Ilaha Illallah - ${botName} 🕋`;

    const botInfo = await global.bot?.getMe?.().catch(()=>null);
    const botUsername = botInfo?.username || global.config?.botInfo?.username || "Eren_AI_Bot";
