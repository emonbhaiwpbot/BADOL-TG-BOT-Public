...
// 2 জন Author Support
const ALLOWED_AUTHORS = ["MOHAMMAD BADOL", "S4Eren"];
...
async function loadCommands(showProgress = true) {
  ...
  if (!fs.existsSync(commandsPath)) {
    ...
    log.warn('Commands directory not found, created: scripts/cmds');
...
async function loadEvents(showProgress = true) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      const event = result.command; if (!event?.config?.name) throw new Error('Missing config');

      // 2 Owner Name Support
      if (!event.config.author) event.config.author = REQUIRED_AUTHOR;
      if (!ALLOWED_AUTHORS.includes(event.config.author)) {
        event.config.author = REQUIRED_AUTHOR;
      }

      if (!event.BADOL) throw new Error('Missing BADOL()'); global.badol.events.set(event.config.name, event); loadedEvents.push(file);
...
async function reloadCommand(commandName) {
  try {
    ...
    if (!command.config.author) command.config.author = REQUIRED_AUTHOR;
    if (!ALLOWED_AUTHORS.includes(command.config.author)) command.config.author = REQUIRED_AUTHOR;
...
async function reloadEvent(eventName) {
  try {
    ...
    delete require.cache[require.resolve(path.join(eventsPath, eventFile))]; const event = require(path.join(eventsPath, eventFile)); if (!event?.config) return { success: false, message: 'Invalid' };
    if (!event.config.author) event.config.author = REQUIRED_AUTHOR;
    if (!ALLOWED_AUTHORS.includes(event.config.author)) event.config.author = REQUIRED_AUTHOR;
    global.badol.events.set(event.config.name, event); return { success: true, message: `Event ${eventName} reloaded` };
  } catch (error) { return { success: false, message: error.message }; }
}

function deleteCommandFile(fileName) { try { const p = path.join(__dirname, '..', 'scripts', 'cmds', fileName); if (!fs.existsSync(p)) return { success: false, message: 'File not found' }; fs.unlinkSync(p); global.badol.commands.delete(fileName.replace('.js', '')); return { success: true, message: `Deleted ${fileName}` }; } catch (e) { return { success: false, message: e.message }; } }
function deleteEventFile(fileName) { try { const p = path.join(__dirname, '..', 'scripts', 'events', fileName); if (!fs.existsSync(p)) return { success: false, message: 'Not found' }; fs.unlinkSync(p); global.badol.events.delete(fileName.replace('.js', '')); return { success: true, message: `Deleted ${fileName}` }; } catch (e) { return { success: false, message: e.message }; } }
...
function installCommandFile(fileName, code) {
  try {
    ...
    if (authorMatch) {
      ...
      if (!ALLOWED_AUTHORS.includes(currentAuthor)) {
        code = code.replace(/author:\s*["'][^"']+["']/, `author: "${REQUIRED_AUTHOR}"`);
      }
    } else {
      if (!code.includes('author')) { code = code.replace(/config:\s*{/, `config: {\n author: "${REQUIRED_AUTHOR}",`); }
    }
    const p = path.join(__dirname, '..', 'scripts', 'cmds', fileName); fs.writeFileSync(p, code); return { success: true, message: `Installed ${fileName}` };
...
function installEventFile(fileName, code) {
  try {
    ...
    const p = path.join(__dirname, '..', 'scripts', 'events', fileName);
